// src/content_scripts/audioManager.js
...
const AudioManager = (function () {
  ...
  const mp3Filename = Constants.getDefaultMp3Filename();
  ...
  const logDebug = (message, ...args) => {
    if (Constants.getDebug()) {
      // Using the getter method from Constants
  ...
  const loadSound = () => {
    chrome.storage.local.get(['selectedSound', 'chimeVolume'], result => {
      cachedSoundFileURL =
        ...
        chrome.runtime.getURL(`resources/${mp3Filename}`);
      cachedVolume =
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
              ...
              .catch(error => {
                ...
                if (
                  ...
                  chrome.storage.local.set(
                    {
                      selectedSound: chrome.runtime.getURL(
                        `resources/${mp3Filename}`
                      ),
                    ...
                    () => {
                      if (chrome.runtime.lastError) {
                        console.error(
                          'Error reverting to default sound:',
                      ...
                      cachedSoundFileURL = chrome.runtime.getURL(
                        `resources/${mp3Filename}`
                      );
                      preloadAudio();
          ...
          audio.onerror = event => {
            ...
            // If the current sound is custom, attempt to revert to default
            if (
              soundFileURL !== chrome.runtime.getURL(`resources/${mp3Filename}`)
            ) {
              logDebug(
                'Attempting to revert to default sound due to playback error.'
              );
              audio.onerror = null; // Remove current error handler to prevent recursion
              chrome.storage.local.set(
                {
                ...
                () => {
                  if (chrome.runtime.lastError) {
                    ...
                    chrome.runtime.sendMessage({
                      message: 'audioPlaybackError',
                      error:
                        'Failed to play custom sound. Reverting to default.',
                    });
                    reject(event);
                    return;
                  }
                  ...
                  cachedSoundFileURL = chrome.runtime.getURL(
                    `resources/${mp3Filename}`
                  ); // Use default if no custom sound
                  preloadAudio();
                  playChime(); // Retry with default sound
                  resolve();
                }
...
// Export AudioManager if it needs to be used elsewhere
