// src/content_scripts/options.js
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const chimeVolumeSlider = document.getElementById('chimeVolumeSlider'); // Define chimeVolumeSlider
  const volumeValueDisplay = document.getElementById('volumeValueDisplay'); // Define volumeValueDisplay
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    switch (request.message) {
      case MESSAGES.AUDIO_PLAYBACK_ERROR:
  ...
  if (disableButton) {
    disableButton.addEventListener('click', () => {
      if (
        confirm(
          'Are you sure you want to disable the Vet Radar Notification extension?'
        )
      ) {
        chrome.runtime.sendMessage(
          { message: MESSAGES.TOGGLE_EXTENSION_STATE, state: false },
          () => {
      ...
      if (file && (file.type === 'audio/mpeg' || file.type === 'audio/mp3')) {
        ...
        reader.onload = function (e) {
          ...
          chrome.storage.local.set(
            { selectedSound: soundData, soundName: file.name },
            () => {
              if (chrome.runtime.lastError) {
              ...
              showMessage(
                'Sound file selected. Click "Save and Close" to apply.',
                'success'
              );
            }
          );
