...
import { DOMHandlers } from './domHandlers.js'; // Import domHandlers
  ...
  switch (request.message) {
    case MESSAGES.PLAY_TEST_CHIME:
      try {
        if (AudioManager && typeof AudioManager.playChime === 'function') {
          ...
          sendResponse({
            success: true,
            message: 'Chime played successfully.',
          });
        } else {
          console.error(
            'AudioManager is not defined or playChime is not a function.'
          );
          sendResponse({
            success: false,
            error:
    ...
    case MESSAGES.TOGGLE_DEBUG:
      try {
        ...
        sendResponse({
          success: true,
          message: `Debug mode set to ${request.debug}`,
        });
      } catch (error) {
    ...
    case MESSAGES.OUTPUT_PATIENT_LISTS:
      try {
        ...
        sendResponse({
          success: true,
          message: 'Patient lists logged successfully.',
        });
      } catch (error) {
    ...
    case MESSAGES.REINITIALIZE_AND_OUTPUT_PATIENT_LIST:
      try {
        ...
        sendResponse({
          success: true,
          message: 'Patient list reinitialized and logged successfully.',
        });
      } catch (error) {
